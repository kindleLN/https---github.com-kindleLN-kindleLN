...
def activateEmailView(r, email, step):
    email_obj = get_object_or_404(EmailModel, email=email, active=False)
    ...
    if r.method == 'GET':
        ...
        if int(code) == email_obj.code:
            email_obj.active = True
            email_obj.save()

            return render(r, 'user/activate_success.html', {'time': datetime.datetime.now().strftime('%Y-%m-%d'), 'email': email})
        else:
            try:
                if (timezone.now() - email_obj.last_send_code_time).seconds <= 600:
                    dct = {
