...
def doCheckUpdateRequest():
    ...
    latest_req = CheckUpdateRequestModel.objects.filter(done=False).order_by(
        "request_time"
    )
    if not latest_req:
        return
    ...
    if not book.has_static_files or source_name == "wenku8":  # wenku8全下了
        ...
        if linovelib_vols:
            try:
                data = lnd_main.main(
                    int(book_id_in_source),
                    "linovelib",
                    download_all_volumes=False,
                    download_vol_names=linovelib_vols,
                )
            except Exception as e:
                raise
            finally:
        ...
        # 最后更新
            ...
            timezone.now().replace(tzinfo=None) - last_update_in_linovelib
        ).days <= 30:
            try:
                data = lnd_main.main(
                    int(book_id_in_source),
    ...
    for i in data:
        ...
        vol_obj = VolumeModel.objects.create(
            name=vol_name, book=book, file_id=file_obj.id
        )
        vol_obj.save()
...
def makeCheckUpdateRequset(book_boj):
    cur = CheckUpdateRequestModel.objects.create(
        book=book_boj, done=False, done_time=None
    )
    cur.save()
...
def makePushRequest(email, file_id):
    email_obj, b = EmailModel.objects.get_or_create(email=email.lower())
...
def handleUploadFilesForLnd(file):
    ...
    FileModel.objects.create(
        name=unique_name, digest=digest_obj, size=os.path.getsize(file)
    )
    # 重命名文件
...
def getLinovelibLastUpdateDate(id):
    ...
    update_time = datetime.datetime.strptime(date, "%Y-%m-%d %H:%M:%S").replace(
        tzinfo=None
    )
...
def makeEpubFile(vol):
    ...
    if vol.parent.source == "linovelib":
        file_name = "[%s]%s-%s[linovelib].epub" % (
            vol.volume_id,
            vol.name,
            vol.parent.name,
        )
...
"""
    ...
    with open(
        os.getcwd() + "\\epubFile\\OEBPS\\content.html", "w", encoding="utf-8"
    ) as f:
...
<manifest>
    ...
    with open(
        os.getcwd() + "\\epubFile\\OEBPS\\content.opf", "w", encoding="utf-8"
    ) as f:
        f.write(
            html
            % (
        ...
        for i in string[:]:
            if emoji.demojize(i) != i:  # 是emoji的单字
                if i not in vol.emojis:
                    vol.emojis.append(i)
                    vol.emojis_file.append(
                        File(getEmojiUrl(i), "emoji%i.png" % vol.emojis.index(i))
                    )
...
def getEmojiUrl(emoji: str):
    ...
    emoji_imgs_soup = soup.find(
        "ul", class_="emoji_imgs imgs_lg row row-cols-1 row-cols-lg-2 mb-0"
    )
...
def getPageHtmlSoup(
    ...
    for i in range(1, retry_times + 1):
        try:
            if print_msg:
                print("[爬取HTML]尝试(%i/%i): %s" % (i, retry_times, url))
            ...
            if use_request:
                ...
                page_html = scraper.get(
                    url, headers=header, timeout=(15.06, 27)
            ...
            print("[爬取HTML]!!警告!!触发反爬机制，休息30s后继续.")
            time.sleep(30)
            print()
